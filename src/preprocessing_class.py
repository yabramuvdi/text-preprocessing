...
class RawDocs():
    ...
    contraction_split=True, tokenization_pattern=None):
        ...
        # stopwords
        # if stopwords == 'long':
        #     self.stopwords = stp_long
        # elif stopwords == 'short':
        #     self.stopwords = stp_short
        ...
        if tokenization_pattern is None:
            ...
            self.tokens = list(map(lambda x:nltk.regexp_tokenize(x,
                            pattern=tokenization_pattern), self.docs))
    ...
    def phrase_replace(self, replace_dict, case_sensitive_replacing=False, items='tokens'):
    ...
    def get_term_ranking(self, items, score_type='df'):
        ...
        if score_type == 'df':
            ...
            def tf_idf_compute(t, scores_tf, scores_df):
                return (1+np.log(scores_tf[t])*np.log(self.N/scores_df[t]))
